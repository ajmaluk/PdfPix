...
export default function PdfFormsPage() {
  ...
  useEffect(() => {
    ...
    const loadPdf = async () => {
      ...
      try {
        ...
        console.error("Failed to load PDF for forms editing:", error);
        alert("Failed to load PDF for forms editing.");
        resetEditor();
      } finally {
        setLoadingPdf(false);
      }
  ...
  const saveChanges = useCallback(async () => {
    ...
    try {
      ...
      downloadBlob(new Blob([bytes as BlobPart], { type: "application/pdf" }), `filled_${files[0].name}`);
    } catch (error) {
      console.error("Failed to save filled PDF Form:", error);
  ...
  const noFileSidebar = (
    <div className="option__panel split-sidebar-panel">
      <div className="split-sidebar-panel__header">
        <div className="option__panel__title split-sidebar-panel__title text-center w-full">PDF Forms</div>
      </div>

      <div className="option__panel__content split-sidebar-panel__content">
        <div className="split-section">
          <label className="text-xs font-bold text-[#8a8a92] uppercase tracking-wider block mb-3">Interactive Form Editor</label>
          <div className="text-xs text-[#555c66] font-medium leading-relaxed">
            Upload a PDF document to fill and edit form fields. Drag, scale, edit text, place images, and draw overlays natively.
          </div>
        </div>
        ...
        <div className="split-section">
          ...
          <div className="space-y-2">
            {["Interactive form field matching", "Automated text injection", "Drag text elements in place", "100% In-browser processing"].map((feature) => (
  ...
  if (files.length === 0) {
    return (
      <ToolLayout
        toolId="forms"
        title="PDF Forms"
        subtitle="Fill out and edit PDF forms easily with interactive drag-and-drop fields."
  ...
  return (
    <>
      ...
      <main className="edit-editor-shell">
        <section className="edit-toolbar">
          <div className="edit-toolbar__primary">
            <div className="edit-toolbar__tools">
              ...
              <button className={tool === "text" ? "is-active" : ""} onClick={() => { setMode("edit"); setTool("text"); }} title="Add text field">
                <Type size={20} />
              </button>
              <button className={tool === "image" ? "is-active" : ""} onClick={() => imageInputRef.current?.click()} title="Add photo or signature">
                <ImagePlus size={20} />
              </button>
              <button onClick={removeSelectedOverlay} disabled={!selectedItem} title="Remove field">
                <Eraser size={20} />
              </button>
            </div>
          </div>
            ...
            <aside className="edit-sidebar edit-sidebar--settings">
              <div className="edit-sidebar__section">
                ...
                <div className="edit-info-card">
                  <Highlighter size={18} />
                  <p>Click on any text block on the page to fill form values, or use the Type tool to place and drag new text values anywhere.</p>
                </div>

                 <div className="edit-settings-block">
                  <label>Form text value</label>
                  <textarea
                ...
                <div className="edit-settings-block">
                  ...
                  <div className="edit-color-row">
                    {COLORS.map((color) => (
                      <button
                        key={color}
                        className={`edit-color-swatch ${textColor === color ? "is-active" : ""}`}
                ...
                <div className="edit-metrics">
                  ...
                  <div><span>Added layers</span><strong>{totalOverlayCount}</strong></div>
                  <div><span>Form fields</span><strong>{totalDetectedTextCount}</strong></div>
                  <div><span>Filled items</span><strong>{editedTextCount}</strong></div>
                </div>

                <button 
                  className="btn-sidebar-cta edit-save-desktop" 
                  onClick={saveChanges} 
                  disabled={processing || loadingPdf}
                  style={{ backgroundColor: "#ab6993" }}
                >
                  <span>{processing ? "Saving Form..." : "Save & Download"}</span>
                  <span className="btn-sidebar-cta__icon">→</span>
                </button>
        ...
        <button 
          className="edit-save-mobile show--sm" 
          onClick={saveChanges} 
          disabled={processing || loadingPdf}
          style={{ backgroundColor: "#ab6993" }}
        >
      ...
      <ProcessOverlay
        ...
        message={loadingPdf ? "Preparing PDF form editor..." : "Saving PDF Form..."}
      />
    </>
  );
}
