...
export default function FileUploader({ onFilesSelected, accept = ".pdf", hasFiles: propHasFiles, fileCount: propFileCount }: FileUploaderProps) {
  ...
  const getButtonLabel = () => {
    ...
    if (lower.includes("doc") || lower.includes("word")) return "Select Word files";
    if (lower.includes("ppt") || lower.includes("powerpoint")) return "Select PowerPoint files";
    if (lower.includes("xls") || lower.includes("xlsx") || lower.includes("csv") || lower.includes("excel")) return "Select Excel files";
    if (lower.includes("jpg") || lower.includes("jpeg") || lower.includes("png") || lower.includes("image")) return "Select images";
  ...
  return (
    <div
      id="uploader"
      className={`uploader ${activeHasFiles ? "box md:hidden" : ""}`}
      onDragOver={(e) => e.preventDefault()}
      onDrop={handleDrop}
      onClick={() => inputRef.current?.click()}
    >
      <div className="uploader__main-row">
        <button type="button" className="uploader__btn tooltip--left border-none outline-none" id="pickfiles" title="Add more files" onClick={(e) => { e.stopPropagation(); e.preventDefault(); inputRef.current?.click(); }}>
        ...
        <div className="uploader__extra">
          {activeHasFiles && (
            <button type="button" className="btn-icon uploader__extra__btn tooltip tooltip--right active border-none outline-none" id="uploadDisk" title="Upload from your computer" onClick={(e) => { e.stopPropagation(); e.preventDefault(); inputRef.current?.click(); }}>
          ...
          <button type="button" className="btn-icon uploader__extra__btn tooltip tooltip--right active border-none outline-none" id="gdrive_file_selector" title="Select from Google Drive" onClick={(e) => e.preventDefault()}>
          ...
          <button type="button" className="btn-icon uploader__extra__btn tooltip tooltip--right active border-none outline-none" id="dropbox_file_selector" title="Select from Dropbox" onClick={(e) => e.preventDefault()}>
